...
export class Pointer {
  ...
  private checkOffset(offset: number) {
    ...
    if (offset > this.uint8Array.length) {
      throw new RangeError(
        `Offset ${offset} is past the end of array of length ${this.uint8Array.length}`,
      );
    }
  }
  private checkEnd(bytes: number) {
    if (this._offset + bytes > this.uint8Array.length) {
      throw new RangeError(
        `Attempted to read past end of Uint8Array of length ${this.uint8Array.length}`,
      );
    }
  }
  byte() {
  ...
  view(numberOfBytes: number | bigint) {
    ...
    const result = this.uint8Array.subarray(
      this._offset,
