...
async function* parseFrequencies(
  lines: AsyncIterable<string>,
): AsyncIterable<[string, number]> {
  for await (const line of lines) {
    const match = line.match(
      /(?:^|\s)(?<word>\p{Letter}+)\s+(?<frequency>\d+(?:,\d{3})*(?:\.\d+)?)\b/u,
    );
    if (match) {
      yield [
        match.groups!.word!,
...
async function parseWordsAndFrequenciesMap(
  wordsFile: string,
  ...
  if (topN < Infinity) {
    const wordsWithFrequencies = parseFrequencies(
      readFileLines(frequenciesFile || wordsFile),
    );
...
async function filterByFrequency({
  ...
  wordsWithFrequencies.sort(
    ([wordA, frequencyA], [wordB, frequencyB]) => frequencyB - frequencyA,
  );
...
async function dictc({
  ...
  const { words: allWords, frequenciesMap } = await parseWordsAndFrequenciesMap(
    wordsFile,
    frequenciesFile,
  );
  const words = await filterByFrequency({
    words: allWords,
    topN,
    frequenciesMap,
  });
  ...
  if (languageCode !== undefined)
    lexicon.metadata?.languageCodes?.push(languageCode);
