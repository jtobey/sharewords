...
export class PointerHandler {
  ...
  private updateTransform() {
    ...
    const maxPanX =
      (this.scale * boardRect.width - boardRect.width) / this.scale;
    const maxPanY =
      (this.scale * boardRect.height - boardRect.height) / this.scale;
  ...
  pointerCancel(evt: PointerEvent) {
    ...
    if (info)
      console.debug(
        `Pointer cancel: ${evt.pointerId} (${info.x.toFixed(2)},${info.y.toFixed(2)})`,
      );
  ...
  pointerMove(evt: PointerEvent) {
    ...
    if (
      !info.pointerMoved &&
      Math.hypot(evt.clientX - info.downX, evt.clientY - info.downY) > 5
    ) {
    ...
    if (!info.draggingTile) {
      ...
      const panningPointerInfos = [
        ...this.pointerInfoMap
          .values()
          .filter((anyInfo) => !anyInfo.draggingTile),
      ];
      const panningPointerCount = panningPointerInfos.length;
      const midpointBefore = panningPointerInfos.reduce(
        (sum, curr) => {
          return {
      ...
      const getMaxDistance = (
        ...
        return Math.max(
          ...points.map((p) => Math.hypot(midpoint.x - p.x, midpoint.y - p.y)),
        );
      };
      const maxDistanceBefore = getMaxDistance(
        midpointBefore,
        panningPointerInfos,
      );
      const midpointAfter = {
    ...
    if (info.draggingTile && info.ghostTile) {
      ...
      if (dropTarget) {
        try {
          ...
          const selectedPlacement = this.gameState.tilesHeld.find(
      ...
      (evt.target as HTMLElement).closest("#board-container") &&
      !info.pointerMoved
    ) {
      const now = Date.now();
      if (now - this.lastTap < 300) {
        ...
        if (this.scale > 1) {
          ...
          this.scale = 1.8;
          const boardRect = this.view
            .getBoardContainer()
            .getBoundingClientRect();
          const square = (evt.target as HTMLElement)
            .closest(".square")
            ?.getBoundingClientRect();

          function toBoardCoordinate(
            evtCoordinate: number,
            boardLo: number,
            boardHi: number,
            squareLength: number,
          ) {
          ...
          const boardX = toBoardCoordinate(
            evt.clientX,
            boardRect.left,
            boardRect.right,
            square ? square.right - square.left : 0,
          );
          const boardY = toBoardCoordinate(
            evt.clientY,
            boardRect.top,
            boardRect.bottom,
            square ? square.bottom - square.top : 0,
          );
