...
function createMockView(browser: TestBrowser): View {
  ...
  return {
    ...
    clearDropTarget: mock((id: number) => {
      dropTargetMap.delete(id);
    }),
    ...
    setDropTarget: mock((id: number, row: TilePlacementRow, col: number) => {
      dropTargetMap.set(id, { row, col });
    }),
  } as unknown as View;
}
...
describe("pointer handler", () => {
  describe("tile dragging", () => {
    ...
    function dragAndDrop(fromElement: HTMLElement, toElement: HTMLElement) {
      ...
      handler.pointerMove(
        createMockPointerEvent(fromElement, fromRect.x + 10, fromRect.y),
      );
      ...
      handler.pointerMove(
        createMockPointerEvent(toElement, toRect.x, toRect.y),
      );
  ...
  describe("double tap", () => {
    test("double tap zooms in", () => {
      ...
      expect(view.setBoardTransform).toHaveBeenCalledWith(
        1.8,
        expect.closeTo(-44.444),
        expect.closeTo(-44.444),
      );
      setSystemTime();
    });
    ...
    test("panning moves the board when zoomed in", () => {
      ...
      expect(view.setBoardTransform).toHaveBeenLastCalledWith(
        1.8,
        expect.closeTo(-55.5555),
        expect.closeTo(-61.1111),
      );
  ...
  describe("long tap", () => {
    test("long tap on a tile shows info popup", () => {
      ...
      expect(view.showInfoPopup).toHaveBeenCalledWith(
        ["word1", "word2"],
        square,
      );
