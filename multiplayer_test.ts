...
describe("multi-player", () => {
  it("should allow two players to take turns", async () => {
    ...
    expect(JSON.stringify(app2.gameState.shared)).toEqual(
      JSON.stringify(app1.gameState.shared),
    );
    ...
    expect(JSON.stringify(app1.gameState.shared)).toEqual(
      JSON.stringify(app2.gameState.shared),
    );
  });
  ...
  it("should allow tab closure between turns", async () => {
    ...
    // Now, player 1's game state should be synced with player 2's
    expect(reloadedApp1.gameState.shared.toJSON()).toEqual(
      app2.gameState.shared.toJSON(),
    );
  });
  ...
  it("should handle collisions in pending placements", async () => {
    ...
    const p1_tile1_placement = app1.gameState.tilesHeld.find(
      (p) => p.row === "rack",
    )!;
    app1.gameState.moveTile(
      p1_tile1_placement.row,
      p1_tile1_placement.col,
      7,
      7,
    );
    const p1_tile2_placement = app1.gameState.tilesHeld.find(
      (p) => p.row === "rack",
    )!;
    app1.gameState.moveTile(
      p1_tile2_placement.row,
      p1_tile2_placement.col,
      7,
      8,
    );
    ...
    const p2_tile1_placement = app2.gameState.tilesHeld.find(
      (p) => p.row === "rack",
    )!;
    app2.gameState.moveTile(
      p2_tile1_placement.row,
      p2_tile1_placement.col,
      7,
      7,
    );
    const p2_tile2_placement = app2.gameState.tilesHeld.find(
      (p) => p.row === "rack",
    )!;
    app2.gameState.moveTile(
      p2_tile2_placement.row,
      p2_tile2_placement.col,
      8,
      7,
    );
    await app2.gameState.playWord();
    ...
    const final_p1_tile1_placement = app1.gameState.tilesHeld.find(
      (p) => p.tile === p1_tile1_placement.tile,
    );
    expect(final_p1_tile1_placement?.row).toBe("rack");

    // 3. Player 1's other placed tile remains on its square.
    const final_p1_tile2_placement = app1.gameState.tilesHeld.find(
      (p) => p.tile === p1_tile2_placement.tile,
    );
  ...
  it("should recover from an out-of-order turn", async () => {
    ...
    expect(browser1.getDocument().querySelector(selector)).toBeTruthy();
    expect(
      app1.gameState.tilesHeld.find((p) => p.row === 8 && p.col === 8),
    ).toBeTruthy();
  });
  ...
  describe("player name change", () => {
    it("should sync player name changes", async () => {
      ...
      expect(app2.gameState.players[1]!.name).toBe(
        app1.gameState.players[1]!.name,
      );
    });
