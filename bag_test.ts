...
describe("bag", () => {
  it("should shuffle", () => {
    const bag = new Bag({tiles: _tiles(1, 2, 3, 5, 8), ..._prng()})
  ...
  it("should support duplicates", () => {
    const bag = new Bag({tiles: _tiles(3, 3, 3, 3), ..._prng(99)})
  ...
  it("should support an empty bag", () => {
    const bag = new Bag({tiles: _tiles(), ..._prng(12)})
    expect(bag.toJSON()).toEqual({prng: {seed: 12}, tiles: []})
  })
  describe("json", () => {
    it("should roundtrip to and from JSON", () => {
      const bag = new Bag({tiles: _tiles(1, 2, 3, 5, 8), ..._prng()})
      const bag2 = Bag.fromJSON({json: bag.toJSON(), ..._constructors()})
      expect(bag2).toEqual(bag)
      expect(bag2.draw(1)).toEqual(_tiles(8))
      expect(bag2).not.toEqual(bag)
    })
    it("should deserialize an empty bag", () => {
      const bag = Bag.fromJSON({json: {prng: {seed: 7}, tiles: []}, ..._constructors()})
      expect(bag.toJSON()).toEqual({prng: {seed: 7}, tiles: []})
    })
    it("should deserialize a non-empty bag", () => {
      const bag = Bag.fromJSON({json: {prng: {seed: 7}, tiles: [35]}, ..._constructors()})
      expect(bag.toJSON()).toEqual({prng: {seed: 7}, tiles: [35]})
    })
    it("should reject an invalid object", () => {
      expect(() => Bag.fromJSON({json: 'frob', ..._constructors()})).toThrow(TypeError)
    })
    it("should reject a non-numeric seed", () => {
      expect(() => Bag.fromJSON({json: {prng: {seed: 'x'}, tiles: []}, ..._constructors()})).toThrow(TypeError)
    })
    it("should reject a non-array tiles", () => {
      expect(() => Bag.fromJSON({json: {prng: {seed: 123}, tiles: null}, ..._constructors()})).toThrow(TypeError)
    })
    it("should reject an invalid tile", () => {
      expect(() => Bag.fromJSON({json: {prng: {seed: 7}, tiles: [null]}, ..._constructors()})).toThrow(TypeError)
    })
  })
})
