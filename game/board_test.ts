...
function _t(
  ...
  letter: string,
  value: number,
  assignedLetter = null as string | null,
) {
  const tile = {
    row,
    col,
...
describe("board", () => {
  ...
  describe("validation", () => {
    it("should throw if no tiles are provided", () => {
      ...
      expect(() => board.checkWordPlacement()).toThrow(
        "Drag some tiles onto the board, and try again.",
      );
    });
    ...
    it("should throw if tiles are not in a line", () => {
      ...
      `)[0]!;
      const diff = diffBoards(oldBoard!, newBoard!);
      expect(() => oldBoard!.checkWordPlacement(...diff)).toThrow(
        "Tiles are not in a line.",
      );
    });
    ...
    it("should throw if tiles form a line with gaps", () => {
      ...
      const diff = diffBoards(oldBoard!, newBoard!);
      expect(() => oldBoard!.checkWordPlacement(...diff)).toThrow(
        "Tiles form a line with gaps between them.",
      );
    });
    ...
    it("should throw if only a single letter word is formed", () => {
      ...
      const diff = diffBoards(oldBoard!, newBoard!);
      expect(() => oldBoard!.checkWordPlacement(...diff)).toThrow(
        "No single-letter words accepted.",
      );
    });
    ...
    it("should throw if the first word is not on the center square", () => {
      ...
      const diff = diffBoards(oldBoard!, newBoard!);
      expect(() => oldBoard!.checkWordPlacement(...diff)).toThrow(
        "Tiles must connect to existing words or cover the center square.",
      );
    });
    ...
    it("should throw if a blank tile is not assigned a letter.", () => {
      const board = new Board("...");
      expect(() =>
        board.checkWordPlacement(_t(0, 0, "", 0), _t(0, 1, "I", 1)),
    ...
    it("should throw if a non-blank tile is assigned a letter.", () => {
      const board = new Board("...");
      expect(() =>
        board.checkWordPlacement(_t(0, 1, "A", 1), _t(0, 2, "T", 1, "N")),
  ...
  describe("json", () => {
    ...
    it("should reject a missing rows", () => {
      expect(() => Board.fromJSON({ tiles: [], scores: [] })).toThrow(
        TypeError,
      );
    });
    ...
    it("should reject a non-array rows", () => {
      expect(() =>
        Board.fromJSON({ rows: "frob", tiles: [], scores: [] }),
      ).toThrow(TypeError);
    });

    it("should reject a non-string row", () => {
      expect(() =>
        Board.fromJSON({ rows: [null], tiles: [], scores: [] }),
      ).toThrow(TypeError);
    });
    ...
    it("should reject a non-array tiles", () => {
      expect(() =>
        Board.fromJSON({ rows: [], tiles: "frob", scores: [] }),
      ).toThrow(TypeError);
    });

    it("should reject an invalid tile", () => {
      expect(() =>
        Board.fromJSON({ rows: [], tiles: [null], scores: [] }),
      ).toThrow(TypeError);
    });

    it("should reject an invalid tile row", () => {
      expect(() =>
        Board.fromJSON({ rows: [], tiles: [["a", 1, "A:1"]], scores: [] }),
      ).toThrow(TypeError);
    });

    it("should reject an invalid tile col", () => {
      expect(() =>
        Board.fromJSON({ rows: [], tiles: [[0, "b", "A:1"]], scores: [] }),
      ).toThrow(TypeError);
    });
    ...
    it("should reject a short score array", () => {
      expect(() =>
        Board.fromJSON({ rows: [".."], tiles: [], scores: [["p"]] }),
      ).toThrow(TypeError);
    });

    it("should reject a long score array", () => {
      expect(() =>
        Board.fromJSON({ rows: [".."], tiles: [], scores: [["p", 0, 0]] }),
      ).toThrow(TypeError);
    });

    it("should reject a non-string playerId", () => {
      expect(() =>
        Board.fromJSON({ rows: [".."], tiles: [], scores: [[16, 0]] }),
      ).toThrow(TypeError);
    });

    it("should reject a non-numeric score", () => {
      expect(() =>
        Board.fromJSON({ rows: [".."], tiles: [], scores: [["p", "0"]] }),
      ).toThrow(TypeError);
    });
