...
describe("test support", () => {
  ...
  it("should parse headers", () => {
    ...
    expect(parsed[0][1].headers).toEqual(
      new URLSearchParams([["long-header", "hello there!"]]),
    );
    expect(parsed[0][2].squares).toHaveLength(1);
    expect(parsed[0][2].squares[0]).toHaveLength(2);
    expect(parsed[0][2].headers).toEqual(
      new URLSearchParams("disjoint=header"),
    );
  });
  ...
  it("should support multiples in headers", () => {
    ...
    `;
    const parsed = parseBoards(boardsStr);
    expect(parsed[0][0].headers.getAll("mh")).toEqual([
      "value 1",
      "value 2",
      "value 3",
    ]);
  ...
  it("should ignore a blank line after headers", () => {
    ...
    expect(parsed[0][0].headers).toEqual(
      new URLSearchParams("header-name=header-value"),
    );
  });
