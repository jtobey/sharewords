...
export class Settings {
  ...
  static forLanguage(bagLanguage: string) {
    const boardSize = DEFAULT_BOARD_LAYOUT.reduce(
  ...
  static fromJSON(json: any) {
    function fail(msg: string): never {
      throw new TypeError(
        `${msg} in Settings serialization: ${JSON.stringify(json)}`,
      );
    }
    ...
    if (!json.boardLayout.every((s: any) => typeof s === "string"))
    ...
    const dictionaryType =
      json.dictionaryType === "custom" ? "wordlist" : json.dictionaryType;
    if (
      !["permissive", "consensus", "wordlist", "freeapi"].includes(
        dictionaryType,
      )
    ) {
    ...
    settings.letterCounts = checkLetterToNumberMap(
      "letterCounts",
      json.letterCounts,
    );
    settings.letterValues = checkLetterToNumberMap(
      "letterValues",
      json.letterValues,
    );
