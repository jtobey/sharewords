...
describe("honor system bag", () => {
  ...
  describe("json", () => {
    ...
    it("should reject a non-numeric seed", () => {
      expect(() => HonorSystemBag.fromJSON({ tiles: [], prng: "x" })).toThrow(
        TypeError,
      );
    });
    it("should reject a non-array tiles", () => {
      expect(() => HonorSystemBag.fromJSON({ tiles: null, prng: 123 })).toThrow(
        TypeError,
      );
    });
    it("should reject an invalid tile", () => {
      expect(() => HonorSystemBag.fromJSON({ tiles: [null], prng: 7 })).toThrow(
        TypeError,
      );
    });
  });
