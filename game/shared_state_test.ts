...
describe("shared state", () => {
  ...
  test("can create a shared state", () => {
    ...
    const tilesState = new HonorSystemTilesState(
      ...
      { seed: "1" },
      makeTiles({
        letterCounts: settings.letterCounts,
        letterValues: settings.letterValues,
      }),
  ...
  test("throws on bad player ID", () => {
    const settings = Settings.forLanguage("en");
    settings.players = [
      new Player({ id: "2", name: "p1" }),
      new Player({ id: "3", name: "p2" }),
    ];
    expect(() => new SharedState(settings)).toThrow(
      'players[0] should have ID "1", not "2".',
    );
  });
  ...
  test("can play a turn", async () => {
    ...
    const turn = new Turn(player1Id, toTurnNumber(1), {
      playTiles: [
  ...
  test("rejects invalid words", async () => {
    ...
    const player1Id = settings.players[0]!.id;
    const player1Tiles = await sharedState.tilesState.getTiles(player1Id);
    let turn = new Turn(player1Id, toTurnNumber(1), {
      playTiles: [
        { tile: player1Tiles[0]!, row: 7, col: 7 },
        { tile: player1Tiles[1]!, row: 7, col: 8 },
      ],
    ...
    turn = new Turn(settings.players[1]!.id, toTurnNumber(2), {
      playTiles: [{ tile: player2Tiles[0]!, row: 7, col: 9 }],
    });

    expect(sharedState.playTurns(turn)).rejects.toThrow(
      "AAA is not a word in test dictionary. Play rejected.",
    );
  });
  ...
  test("throws on duplicate turn number", async () => {
    ...
    const turn2 = new Turn(player1Id, toTurnNumber(1), {
      exchangeTileIndices: [],
    });
    await expect(sharedState.playTurns(turn, turn2)).rejects.toThrow(
      "playTurns received duplicate turn number 1.",
    );
  });
  ...
  test("throws on wrong player ID", async () => {
    ...
    const turn = new Turn(player2Id, toTurnNumber(1), {
      exchangeTileIndices: [],
    });
    await expect(sharedState.playTurns(turn)).rejects.toThrow(
      'Turn number 1 belongs to player "1", not "2".',
    );
  });
  ...
  test("throws on unsupported dictionary", () => {
    ...
    expect(() => new SharedState(settings)).toThrow(
      "dictionaryType strict is not supported.",
    );
  });
  ...
  test("throws on exchanging too many tiles", async () => {
    ...
    const player1Id = settings.players[0]!.id;
    const turn = new Turn(player1Id, toTurnNumber(1), {
      exchangeTileIndices: [0, 1, 2, 3, 4, 5, 6],
    });
    await expect(sharedState.playTurns(turn)).rejects.toThrow(
      "Player 1 attempted to exchange 7 but the bag holds only 1.",
    );
  });
  ...
  test("throws on invalid turn move", async () => {
    ...
    await expect(sharedState.playTurns(turn)).rejects.toThrow(
      "Turn number 1 is not a play or exchange.",
    );
  });
  ...
  test("throws on unsupported tileSystemType", () => {
    ...
    expect(() => new SharedState(settings)).toThrow(
      "Unsupported tileSystemType: invalid",
    );
  });
  ...
  describe("json", () => {
    test("can serialize and deserialize", () => {
      ...
      expect(sharedState2.tilesState.toJSON()).toEqual(
        sharedState.tilesState.toJSON(),
      );
    });

    test("throws on invalid json", () => {
      expect(() => SharedState.fromJSON(1)).toThrow(
        "Not an object in SharedState serialization: 1",
      );
      expect(() => SharedState.fromJSON({})).toThrow(
        "Wrong keys or key order in SharedState serialization: {}",
      );
      ...
      const template = {
        ...
        tilesState: new HonorSystemTilesState(
          ...
          { seed: "1" },
          makeTiles({
            letterCounts: new Map([["A", 20]]),
            letterValues: new Map([["A", 1]]),
          }),
          7,
        ).toJSON(),
      };
