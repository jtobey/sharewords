...
describe("player", () => {
  ...
  describe("json", () => {
    it("should serialize", () => {
      expect(new Player({ id: "11", name: "Joe" }).toJSON()).toEqual({
        id: "11",
        name: "Joe",
      });
    });
    it("should deserialize", () => {
      expect(Player.fromJSON({ id: "10", name: "Joni" })).toEqual(
        new Player({ id: "10", name: "Joni" }),
      );
    });
    ...
    it("should reject a non-string id", () => {
      expect(() => Player.fromJSON({ id: 13, name: "Aidan" })).toThrow(
        TypeError,
      );
    });
    it("should reject a non-string name", () => {
      expect(() => Player.fromJSON({ id: "10", name: ["Jane"] })).toThrow(
        TypeError,
      );
    });
  });
