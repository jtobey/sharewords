...
export function getBagParam(settings: Settings): string | undefined {
  let bagParam = [...settings.letterCounts.entries()]
    .map(
      ([letter, count]) =>
        `${letter}-${count}-${settings.letterValues.get(letter) ?? 0}`,
    )
    .join(".");
...
export function parseBagParam(bagParam: string, boardLayout: string[]) {
  ...
  for (const config of bagParam.split(".")) {
    if (config.match(/^[a-z]/)) {
      ...
      if (!defaults) {
        throw new UrlError(
          t("error.url.invalid_tile_distribution", { specifier: bagLanguage }),
        );
      }
      ...
      const count =
        g.count === "" || g.count === undefined
          ? undefined
          : parseInt(g.count, 10);
      const value =
        g.value === "" || g.value === undefined
          ? undefined
          : parseInt(g.value, 10);
...
export function parseGameParams(
  ...
  if (vParam && vParam !== settings.version) {
    throw new UrlError(
      t("error.url.protocol_not_supported", { version: vParam }),
    );
  }
  ...
  for (let playerNumber = 1; ; ++playerNumber) {
    ...
    newPlayers.push(
      new Player({
        ...
        name: pnParam.slice(0, settings.maxPlayerNameLength),
      }),
    );
  }
  ...
  if (
    ...
    throw new UrlError(
      t("error.url.unknown_dictionary_type", { type: dtParam }),
    );
  }
  ...
  if (!playerId) {
    ...
    const turnNumber = toTurnNumber(
      urlTurnNumber +
        turnParams.getAll("wl").length +
        turnParams.getAll("ex").length,
    );
