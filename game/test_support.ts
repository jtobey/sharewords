...
function parseRowOfBoards(
  ...
  if (
    !boardLines.every((line) =>
      startColumnsComma.startsWith(`${getStartColumns(line)},`),
    )
  ) {
    throw new Error(`Inconsistent spacing:\n${boardLines.join("\n")}\n`);
  }
  const boardStrings = new Map(
    startColumns.map((startColumn) => [
      startColumn,
      {
        headers: new URLSearchParams(),
  ...
  squares.map((row, rowNumber) => {
    row.map((square, colNumber) => {
      ...
      if (square.assignedLetter)
        boardSquare.assignedLetter = square.assignedLetter;
    });
  });
...
export function diffBoards(a: Board, b: Board): Array<BoardPlacement> {
  if (a.squares.length !== b.squares.length)
    throw new Error(`Mismatched board heights.`);
  if (a.squares[0]?.length !== b.squares[0]?.length)
    throw new Error(`Mismatched board widths.`);
...
function diffSquares(a: Square, b: Square): BoardPlacement | null {
  if (a.tile && b.tile) {
    if (
      ...
      throw new Error(
        `diffBoards: tile or assignment difference at ${a.row},${a.col}: ${JSON.stringify([a, b])}`,
      );
    }
    return null;
  }
