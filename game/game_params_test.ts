...
describe("game params", () => {
  ...
  describe("gameParamsFromSettings", () => {
    ...
    test("all settings", () => {
      ...
      expect(params.toString()).toBe(
        "v=1&p1n=p1&p2n=p2&board=_&bingo=100&bag=A-1-2&racksize=8&seed=foo&dt=freeapi&ds=bar",
      );
    });
  ...
  describe("parseBagParam", () => {
    ...
    test("uses default value", () => {
      ...
      expect(parsed.letterValues.get("A")).toEqual(
        Settings.forLanguage("en").letterValues.get("A")!,
      );
    });
    test("uses default count", () => {
      const parsed = parseBagParam("en.A--3", boardLayout);
      expect(parsed.letterCounts.get("A")).toEqual(
        Settings.forLanguage("en").letterCounts.get("A")!,
      );
