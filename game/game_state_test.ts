...
describe("game state", () => {
  ...
  it("should apply the bingo bonus", async () => {
    ...
    for (const placement of diffBoards(before, after)) {
      const fromCol = gameState.tilesHeld.findIndex(
        (p) => p.row === "rack" && p.tile.equals(placement.tile),
      );
      gameState.moveTile("rack", fromCol, placement.row, placement.col);
    }
  ...
  describe("params", () => {
    ...
    it("should be parsed for non-default settings", async () => {
      ...
      expect(settings.players.map((p) => p.name)).toEqual([
        "player1",
        "player2",
      ]);
      expect(settings.boardLayout).toEqual(["T..", ".d.", "..T"]);
      expect(settings.bingoBonus).toEqual(100);
      expect(settings.letterCounts).toEqual(
        new Map([
          ["A", 10],
          ["B", 10],
        ]),
      );
      expect(settings.letterValues).toEqual(
        new Map([
          ["A", 1],
          ["B", 2],
        ]),
  ...
  it("should return a displaced tile to the rack", async () => {
    ...
    gameState.moveTile("rack", 0, 7, 7);
    expect(gameState.tilesHeld.find((p) => p.row === 7 && p.col === 7)).toBe(
      tileToPlace,
    );

    // Simulate another player's move by placing a tile on the same square
    const otherPlayerTile = {
      tile: new Tile({ letter: "X", value: 8 }),
      row: 7,
      col: 7,
    };
    ...
    expect(
      gameState.tilesHeld.find((p) => p.row === 7 && p.col === 7),
    ).toBeUndefined();
    expect(
      gameState.tilesHeld.find((p) => p.tile.equals(tileToPlace.tile))?.row,
    ).toBe("rack");
  });
