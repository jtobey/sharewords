...
async function handleCommitPlay() {
    ...
    const turnUrlParams = generateTurnUrlParams(currentGame, localPlayerId, wordDataForURL, seedForURL, settingsForURL);
...
function generateTurnUrlParams(game, playerId, turnData, seed = null, settings = null, exchangeData = null) {
    ...
    if (game.turnNumber === 1 && playerId === 'player1') {
        effectiveSeed = game.randomSeed; // Always use current game's seed for P1, Turn 1
        if (effectiveSettings === null) { // If settings weren't explicitly passed (e.g. for pass/exchange)
            effectiveSettings = game.settings; // Use current game's settings
        }
    }
