...
export class GameSetup {
  ...
  constructor(gameState: GameState, browser: Browser) {
    ...
    this.addPlayerButton = this.doc.getElementById(
      "add-player-button",
    )! as HTMLButtonElement;
    this.dictionaryType = this.doc.getElementById(
      "dictionary-type",
    )! as HTMLSelectElement;
    this.dictionaryUrlContainer = this.doc.getElementById(
      "dictionary-url-container",
    )!;
    this.dictionaryUrl = this.doc.getElementById(
      "dictionary-url",
    )! as HTMLInputElement;
    this.tileDistribution = this.doc.getElementById(
      "tile-distribution",
    )! as HTMLSelectElement;
    this.bingoBonus = this.doc.getElementById(
      "bingo-bonus",
    )! as HTMLInputElement;
    this.randomSeed = this.doc.getElementById(
      "random-seed",
    )! as HTMLInputElement;
    this.randomSeedCheckbox = this.doc.getElementById(
      "random-seed-checkbox",
    )! as HTMLInputElement;
    this.startGameButton = this.doc.getElementById(
      "start-game-with-settings",
    )! as HTMLButtonElement;
    this.cancelSettingsButton = this.doc.getElementById(
      "cancel-settings",
    )! as HTMLButtonElement;
  ...
  private _bindSettingsDialogEvents() {
    this.addPlayerButton.addEventListener("click", () => {
      const currentPlayers = Array.from(
        this.playerList.querySelectorAll("input"),
      ).map((i) => ({ name: i.value }));
      currentPlayers.push({ name: "" });
      this._updatePlayerList(currentPlayers);
    });

    this.dictionaryType.addEventListener("change", () =>
    ...
    this.startGameButton.addEventListener("click", () => {
      ...
      settings.players = playerNames.map(
        (name, i) => new Player({ id: String(i + 1), name }),
      );
      ...
      if (dictionaryValue === "permissive") {
        ...
        dictionaryValue === "freeapi" ||
        dictionaryValue === "wordlist"
      ) {
      ...
      if (hasBagDefaults(bagLanguage)) {
        const boardSize = settings.boardLayout.reduce(
  ...
  private _updatePlayerList(players: { name: string; id?: string }[]) {
    ...
    players.forEach((player, index) => {
      ...
      input.placeholder = t("ui.settings.player_placeholder", {
        index: index + 1,
      });
      ...
      if (this.gameState.keepAllHistory && player.id !== undefined) {
        ...
        const gameUrl = this.browser
          .getHref()
          .replace(this.browser.getHash(), "");
  ...
  private _populateSettingsDialog() {
    ...
    const localPlayerIndex = players.findIndex(
      (p) => p.id === this.gameState.playerId,
    );
    ...
    if (
      this.gameState.settings.dictionaryType === "wordlist" &&
      typeof this.gameState.settings.dictionarySettings === "string" &&
      this.dictionaryType.querySelector(
        `option[value="${this.gameState.settings.dictionarySettings}"]`,
      )
    ) {
